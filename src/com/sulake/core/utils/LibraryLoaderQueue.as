package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1357:int = 4;
      ...
      {
         return this.var_403.length + this.var_269.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_269.length < const_1357 && this.var_403.length > 0)
            {
               _loc1_ = this.var_403.shift();
               this.var_269.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_403.indexOf(loader);
            if(index > -1)
            {
               this.var_403.splice(index,1);
            }
            index = this.var_269.indexOf(loader);
            if(index > -1)
            {
               this.var_269.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_403)
            {
               if(param2)
               {
