package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_714:IFigurePartSet;
      ...
      private var var_576:Rectangle;
      
      private var var_1707:int = 0;
      
      private var var_401:IAvatarRenderManager;
      ...
      private const const_1963:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1707 = Math.max(this.var_1707,_loc6_.colorLayerIndex);
            }
         }
         this.var_401 = this.var_64.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_645 != null && !this.var_1709)
            {
               _loc3_ = this.var_645;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_714)
            {
               switch(this.var_714.clubLevel)
               {
                  case HabboClubLevelEnum.const_49:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_62:
                     _loc2_.style = HabboIconType.const_735;
                     break;
                  case HabboClubLevelEnum.const_54:
                     _loc2_.style = HabboIconType.const_858;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_401.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_401.isFigureReady(_loc1_))
         {
            this.var_401.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_323[_loc6_] + "_" + FigureData.const_140;
               _loc4_ = this.var_401.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_576 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1708)
               {
                  ...
                  this.var_1708 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_576.width,this.var_576.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_576.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_576.y;
               if(this.var_1709 && _loc2_.colorLayerIndex > 0)
               {
