package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_837 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_359 = [];
         this.var_615 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_837,this);
               if(_loc9_)
               {
                  this.var_615 = _loc9_.sellableBreeds;
                  this.var_359 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_615.length)
         {
            _loc10_ = this.var_615[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_837,_loc10_.id),this.getRaceLocalizationKey(this.var_837,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_837,_loc5_);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_509 = param1.index;
         if(this.var_509 < 0 || this.var_509 > this.var_359.length)
         {
            this.var_509 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_509 >= 0 && this.var_509 < this.var_359.length)
         {
            _loc7_ = this.var_359[this.var_509] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_615[this.var_463] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_837,_loc3_.id,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_286,AvatarAction.const_642);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_509 >= this.var_359.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_359[this.var_509] as IPetColor;
