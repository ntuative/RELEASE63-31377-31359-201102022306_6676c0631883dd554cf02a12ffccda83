package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1095:Number = 1.0;
      ...
      private var var_1695:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_653:Number = 1.0;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_92 = this.var_1094[param1];
         this.var_868 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1694 >= 0)
         {
            this.var_243 = this._visualization.getSprite(this.var_1694);
            if(this.var_243 && this.var_243.asset)
            {
               ...
               if(this.var_83 == null)
               {
                  this.var_83 = this.var_243.asset.clone();
                  if(this.var_712.alphaMultiplier != 1)
                  {
                     this.var_575 = new BitmapData(this.var_83.width,this.var_83.height,true,4278190080);
                  }
               }
               this.var_2306 = -this.var_243.offsetX;
               this.var_2307 = -this.var_243.offsetY;
               this.var_243.asset = this.var_83;
            }
            if(this.var_83)
            {
            ...
            if(this.var_575)
            {
               this.var_575.fillRect(this.var_575.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_92.y * this.var_1095;
         }
         return 0;
      }
      ...
      {
         if(!this.var_92 || !this.var_243)
         {
            return;
         }
         if(this.var_83 && this.var_243.asset != this.var_83)
         {
            this.var_243.asset = this.var_83;
         }
         if(this.var_868)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_868 && this.var_92.hasIgnited)
         {
            this.var_868 = true;
         }
         _loc8_ = this.var_1164 * this.var_1095;
         this.var_92.update();
         if(this.var_868)
         {
            if(this.var_92.roomObjectSpriteId >= 0)
            ...
            {
               this.var_83.draw(this.var_575,this.var_1695,this.var_712,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_92.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2306 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1095;
               _loc4_ = this.var_2307 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1095;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_83.draw(_loc7_,this._translationMatrix,this.var_1345,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2378 = parseInt(param1.@size);
         this.var_1694 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1164 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1095 = this.var_2378 / 64;
         this.var_653 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_653 = Math.min(this.var_653,1);
         this.var_712.alphaMultiplier = this.var_653;
